...
+++{"lesson_part": "main"}
The control unit is also vital to having a functioning system with a Von Neumann architecture. The control unit tells various components like the memory, ALU, input/output devices how to respond to instructions given to the CPU. The control unit decodes the instructions and supplies them to the various subunits. Control units can be hardwired into the CPU by using physical logic gates to control the flow of the instructions. However this is mostly deprecated due to issues when trying to modify anything about it, a more modern approach is used nowadays of using a micro programmable control unit. It essentially acts as a tiny computer capable of storing some information about instructions that can help get them to their desired location. It is slower than a hardwired CPU, but is much easier to work with. This information was for on [GeeksForGeeks](https://www.geeksforgeeks.org/introduction-of-control-unit-and-its-design/)
